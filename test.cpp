...
// void loadVehicle(Car &carData, Bike &bikeData) {
// 	string type, plate, line;
//     int count = 0;
//     system_clock::time_point time;
//     ifstream parkinglog("parkinglog.txt");
//     if (!parkinglog.is_open()) {
// 		cout << endl;
//         cout << "No previous patient data found, Starting Fresh." << endl;
//     }
//     while (count < carData.slots && getline(parkinglog, line)) {
//         stringstream ss(line);
//         ss >> type >> plate >> time;
//         if(type == "Car"){
//             carData.plates[count] = plate;
//             carData.carEntry[count] = time;
//         } else if(type == "Bike"){
//             bikeData.plates[count] = plate;
//             bikeData.bikeEntry[count] = time;
//         }
//         count++;
//     }
//     parkinglog.close();
...
void parkBike(Bike &bikeData, system_clock::time_point baseTime) {
    ...
    for (int i = 0; i < bikeData.slots; i++) {
        if (bikeData.plates[i] == "") {
            ...
            parkingfile << "Bike" << " " << plate << " " << bikeTime;
            parked = true;
            break;
        }
    }
